#!/bin/bash
# Emergency SSR fix for production deployment
...
echo "🔧 Emergency SSR fix for hollowpress.graveyardjokes.com"
...
# Start SSR server
echo "🚀 Starting SSR server on port 13720..."
...
if kill -0 $SSR_PID 2>/dev/null; then
    ...
    # Test endpoint
    if curl -s -f "http://127.0.0.1:13720" > /dev/null; then
        echo "✅ SSR endpoint responding on port 13720"
